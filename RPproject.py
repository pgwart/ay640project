...
def star_formation_blackbody(f, t1, t2, SFR=None, tau=None):
    tstart = np.max([t1,t2])
    tstop = np.min([t1,t2])
    ...
    if SFR:
        BB = [-1*SFR(t,tau)*gal_blackbody(f, M)*dt(M) for (t,M) in zip(time, Mmax)]
    else:   
        BB = [-1*gal_blackbody(f, M)*dt(M) for (t,M) in zip(time, Mmax)]
    return np.trapz(BB,x=Mmax)

def generate_blackbody(farray, t1, t2, SFR=None, tau=None):
    res = []
    for f in farray:
        B = star_formation_blackbody(f, t1, t2, SFR=None, tau=None)
        if B == np.nan:
            B = 0
        res.append(B)
    return res
